...
//import com.example.demo.utils.AwsService;
...
public class ItemController {
    ...
    public ItemController(ItemProvider itemProvider, ItemService itemService, JwtService jwtService, S3Uploader s3Uploader){
    ...
    public BaseResponse<String> postHeart(@RequestBody PostCommentReq postCommentReq) throws BaseException {
        if (postCommentReq.getContent().length() < 5) {
            return new BaseResponse<>(POST_ITEMS_DESCRIPTION_INVALID);
        }
        try{
    ...
    /**
     * 이미지 업로드 API
     * [POST] /items/:itemIdx/images
     * @return BaseResponse<String>
     */
    // Path-variable
    @ResponseBody
    @PostMapping("/{itemIdx}/images") // (GET) 127.0.0.1:9000/app/items/:itemIdx/images
    public BaseResponse<String> uploadImage (@RequestParam("data")MultipartFile multipartFile, @PathVariable("itemIdx") int itemIdx) throws IOException {

            String result = s3Uploader.upload(multipartFile, "item", itemIdx);
            return new BaseResponse<>(result);
